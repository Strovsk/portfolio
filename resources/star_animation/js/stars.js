window.onload = ()=>{
    // let estrela = new Star({w: 75, h: 75}, {x:100, y:100}, false);
    // let container = document.getElementById("container");
    // container.appendChild(estrela.get_star());
    // console.log(container);
    let stars = new Constelation(
        quantity=15,
        size={
            max: 50,
            min: 15
        ...
        use_container_metrics_width = true,
        use_container_metrics_height = true,
        opacity_status = true
    )
...
class Star{
    constructor(size, position, opacity_status=false, delay={min:1, max:15}, duration={min:1, max:6}){
...
class Constelation{
    constructor(quantity=40, size={max:75, min:20}, viewbox={width:400, height:400, ox:0, oy:0}, container_id, use_container_metrics_width=true, use_container_metrics_height=true, opacity_status=false){
