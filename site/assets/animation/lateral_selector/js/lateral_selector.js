class lateral_selector {
  constructor(container_id='lateral_selector', default_active_node=1) {
      ...
      (entries, observer) => {
        entries.forEach(entrie => {
          if (entrie.isIntersecting) {
            this.update_active_selector(this.targetsList[`#${entrie.target.id}`] | 0);
          }
        });
      },
      {
    ...
    add_selector(text="default", text_horizontal_distance_inactive = 100, text_horizontal_distance_active=100, fontSizeActive=13, fontSizeInactive=13, href) {
      ...
      this.targetsList[href] = parseInt(generic_selector.id.slice(-1));
      ...
      this.observer.observe(document.querySelector(href));
    }
  ...
  var init_selectors = () => {
    ...
    selector_obj.add_selector('Mockups', -80, 0, 10, 13, '#mockups');
